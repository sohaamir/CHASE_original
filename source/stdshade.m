function [lineOut, fillOut] = stdshade(amatrix,alpha,acolor,F,smth,type,malpha)
% usage: stdshading(amatrix,alpha,acolor,F,smth,type,malpha)
...
% - NB: mcolor defines alpha of mean
% smusall 2010/4/23
...
if exist('type','var'); if isempty(type); type='std'; end % <- NB: added type as additional input
else type='std'; %no smoothing by default
end   
if exist('malpha','var'); if isempty(malpha); malpha=1; end % <- NB: added alpha for mean as additional input
else malpha=1; %no smoothing by default
end   
if ne(size(F,1),1)
    F=F';
end
...
switch type
    case 'std', astd = nanstd(amatrix,[],1); % to get std shading
    case 'sem', astd = nanstd(amatrix,[],1)/sqrt(size(amatrix,1)); % to get sem shading
end
...
hold on;
lineOut = plot(F,amean, 'color', [acolor malpha],'linewidth',1.5); %% change color or linewidth to adjust mean line
if check
    hold off;
end
end
